...
SELECT ti.name FROM actor AS ti
JOIN casting AS cl ON ti.id = actorid
JOIN movie AS t ON t.id=movieid
  WHERE movieid=11768
...
SELECT t.title FROM actor AS ti
Join casting AS cl ON ti.id = actorid
Join movie AS t ON t.id=movieid
  WHERE actorid = '2216'
...
SELECT title, name FROM actor AS ti
  JOIN casting AS cl ON ti.id = actorid
  JOIN movie AS t ON t.id=movieid
